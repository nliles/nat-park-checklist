...
const formatParks = (parks: Park[], selectedItem?: ParkDesignationType) => {
  ...
  const formattedParks = selectedItem && PARK_INFO[selectedItem] ? PARK_INFO[selectedItem].formattedParks || [] : ALL_FORMATTED;
  const updatedParks = selectedItem && PARK_INFO[selectedItem] ? (PARK_INFO[selectedItem].parkOverrides || []) : ALL_OVERRIDES;
